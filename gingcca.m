function [Yrows nclasses] = gingcca(A,tol)
...
[nj nk] = size(A);
Ycols = zeros(1,nk); % preliminary labels for each column
Yrows = zeros(nj,1); % preliminary labels for each row
...
for k = 1:nk
    ...
    Cols{k} = find(A(:,k) > tol); % all the non-zero row indices in column k
end
disp('found all non-zeros in columns');
